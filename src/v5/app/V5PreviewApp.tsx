...
function errorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "Workout Core failed";
}
    ...
    .flatMap((exercise) =>
      [...exercise.sets]
        ...
        .map((prescription) => ({
          ...
          exerciseName:
            exercise.exerciseId === PREVIEW_EXERCISE_ID ? PREVIEW_EXERCISE_NAME : exercise.exerciseId,
          prescription,
        })),
    );
}
...
export function V5PreviewApp({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const program = await createProgramRepository().get(resumed.session.programVersionId);
        if (!program) {
  ...
  }, [canRunWorkoutCore, databaseInfo, workoutMode]);
  ...
  async function beginWorkout(): Promise<void> {
    ...
    try {
      ...
      await programs.save(PREVIEW_PROGRAM);
      const sessions = createSessionRepository();
      const session = await startWorkout({
        programVersion: PREVIEW_PROGRAM,
        sessionKey: "upper-a",
        sessions,
        ids: browserIds,
        clock: browserClock,
      });
      setWorkout({ session, sets: [], program: PREVIEW_PROGRAM });
    } catch (error: unknown) {
      setWorkoutError(errorMessage(error));
    } finally {
        ...
        <>
          ...
          {workoutMode ?? (
            <section aria-label="Workout Core">
              {!workoutReady && <p>Restoring workout…</p>}
